 ...
 * @returns
 */
export function convertSeverityToLevel(
  input?: string | LogSeverity,
): LogLevel | undefined {
 ...
 * @returns
 */
export function convertLoggableValueToIJson(
  input: TLoggableValue | TLoggableValue[],
): TJsonValue | TJsonValue[] {
  if (isArray(input)) {
    return input.map((entry) => convertLoggableValueToJsonValue(entry));
  } else {
 ...
 * @returns
 */
export function convertToJsonValue(
...
export function loadJsonFile(filepath: string): IJson | undefined {
  if (!filepath.endsWith(".json")) {
    localDebug(
      `loadJsonFile not loading ${filepath} as it doesn't end with .json`,
    );
    return undefined;
  }

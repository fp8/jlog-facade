...
export function localError(message: string, error?: Error): void {
  if (SHOW_ERROR_MESSAGE) {
    if (error) {
      printLine(`[jlog-facade ERROR] ${message} ${error.toString()}`);
    } else {
      printLine(`[jlog-facade ERROR] ${message}`);
    }
  }
}
  ...
  if (process.env.DEBUG) {
    ...
    if (typeof input === "string") {
      ...
      try {
        ...
        printLine(`[D ERROR] ${e}.  Failed to create log`);
      }
    }
...
export function safeStringify(input: unknown): string {
  try {
    ...
    localDebug(
      () =>
        `safeStringify: JSON.stringify error: ${error} will attempt to remove circular structure and replace with it with "${CIRCULAR_STRUCTURE_ERROR}" string`,
    );
...
/**
 * Write to stdout (Node.js) or console (browser) if input is not empty with a newline at the end.
 *
 * This function works in both Node.js and browser environments.
 *
 * @param input
 */
export function printLine(input: string): void {
  if (input) {
    ...
    if (
      ...
      // Browser environment - fallback to console.log as the base output method
      console.log(input);
    }
  }
}

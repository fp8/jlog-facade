...
function mergeKV(
  mergeValue: boolean,
  kvs: AbstractKeyValue<TLoggableValue>[],
): IJson {
  ...
  for (const kv of kvs) {
    ...
    if (mergeValue === true && key in result) {
      ...
      if (isArray(converted)) {
        converted.forEach((convertedEntry) => merged[key].add(convertedEntry));
      } else {
 ...
 * @returns
 */
export function mergeLoggableModels<T extends TLoggableValue>(
...
export class KV<T extends TLoggableValue> extends AbstractKeyValue<T> {
   ...
   * @returns
   */
  public static merge<T extends TLoggableValue>(
    ...kvs: AbstractKeyValue<T>[]
  ): IJson {
   ...
   * @returns
   */
  public static mergeValue<T extends TLoggableValue>(
    ...kvs: AbstractKeyValue<T>[]
  ): IJson {
...
export class Tags<T extends TLoggableValue> extends AbstractKeyValue<T> {
   ...
   * @returns
   */
  public static of<T extends TLoggableValue>(
    key: string,
    ...values: T[]
  ): Tags<T> {

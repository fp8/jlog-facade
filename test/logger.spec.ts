...
function getLoggablesFromLogEntry(
  entry: Partial<IJLogEntry>,
): AbstractLoggable[] {
...
describe("logger", () => {
  ...
  it("info async log - cb with error", async () => {
    dest.addDestination(new TestAsyncDestination());
    logger.info(() => {
      throw new Error("Error type lH37czD9jC");
    });
    await logger.logWritten();
    expect(logCollector).to.eql([
      "ASYNC-info: log message callback threw error Error type lH37czD9jC",
    ]);
  });
  ...
  it("3 destinations with LoggerFactory", async () => {
    ...
    await logger.logWritten();
    expect(logCollector).to.eql([
      "SYNC-error: uxqEbnCXeZ",
      "STREAM-error: uxqEbnCXeZ",
      "ASYNC-error: uxqEbnCXeZ",
    ]);
    ...
    await logger.logWritten();
    expect(logCollector).to.eql([
      "SYNC-panic: 6gaabROqBx",
      "ASYNC-panic: 6gaabROqBx",
    ]);
  ...
  it("Test Error with Message", () => {
    ...
    const logData = buildOutputDataForDestination(
      entry.loggables,
      entry.data,
      entry.values,
    );
  ...
  it("Test Error with multiple entries", () => {
    ...
    expect(customError.stack).to.satisfies((s: string) =>
      s.startsWith("Error: p4e52JDLvY"),
    );
  });

...
describe("models", () => {
  ...
  it("SimpleTags", () => {
    ...
    expect(entry2.toIJson()).to.eql({
      testTags2: ["WL5oW2Vhll", "WL5oW2Vhll"],
    });
  });
  ...
  it("simple KVLabel", () => {
    ...
    expect(entry.toIJson()).to.eql({
      testKVLabel: { testLabel: [8090, 6860] },
    });
  });

  it("simple StringLabel", () => {
    const entry = new StringLabel(
      "testStringLabel",
      "dDVfo39D3f",
      "HCJejdQNBT",
    );
    expect(entry.toIJson()).to.eql({
      testStringLabel: ["dDVfo39D3f", "HCJejdQNBT"],
    });
  });

  it("simple Loggable", () => {
    const entry = Loggable.of("testLoggable", {
      today: new Date("2023-03-24"),
      weekday: "FRI",
    });
    expect(entry.toIJson()).to.eql({
      testLoggable: {

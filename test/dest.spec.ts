import { KV, LoggerFactory, LogLevel, Loggable } from "@fp8proj";
...
describe("dest", () => {
  ...
  describe("text", () => {
    ...
    it("simple error with message", () => {
      ...
      expect(logCollector).is.eql([
        "|E WryAUrSVas process did not work [Error:WryAUrSVas failed]",
      ]);
    });
    ...
    it("warn with payload", () => {
      LoggerFactory.addLogDestination(new TestSimpleTextDestination());
      logger.warn(
        "This is warning of k2haHZGtKy",
        new KV("entry", "NmAlwOI6D7"),
      );
      expect(logCollector).is.eql([
        '|W This is warning of k2haHZGtKy {"entry":"NmAlwOI6D7"}',
      ]);
    });
  });
  ...
  describe("json", () => {
    ...
    it("simple debug", () => {
      LoggerFactory.addLogDestination(
        new TestSimpleJsonDestination(LogLevel.DEBUG),
      );
    ...
    it("info with payload", () => {
      LoggerFactory.addLogDestination(new TestSimpleJsonDestination());
      logger.info(
        "Info message for UurqWHYMyJ",
        new KV("processId", "UurqWHYMyJ"),
      );
      expect(logCollector).is.eql([
        '{"m":"I|Info message for UurqWHYMyJ","processId":"UurqWHYMyJ"}',
      ]);
    });

    it("error", () => {
      LoggerFactory.addLogDestination(
        new TestSimpleJsonDestination(LogLevel.INFO, false),
      );
      const error = new Error("vOGQbtxvfD");
      logger.error(error);
      expect(logCollector).is.eql([
        '{"m":"E|vOGQbtxvfD","e":"Error: vOGQbtxvfD"}',
      ]);
    });
    ...
    it("error with stack", () => {
      ...
      expect(json.s).satisfies((message: string) =>
        message.startsWith("Error: qbsKviHUSV"),
      );
    });

    it("LogLevel INFO", () => {
      LoggerFactory.addLogDestination(
        new TestSimpleJsonDestination(LogLevel.WARNING),
      );
    ...
    it("LogLevel PANIC", () => {
      LoggerFactory.addLogDestination(
        new TestSimpleJsonDestination(LogLevel.PANIC),
      );
      ...
      expect(logCollector).is.eql(['{"m":"P|Panic message for fYKShPyBvI"}']);
    });
    ...
    it("logger level override - ", () => {
      ...
      LoggerFactory.addLogDestination(
        new TestSimpleJsonDestination(LogLevel.INFO),
      );
      const logger = LoggerFactory.getLogger(
        "logger-ThBC4wm1OJ",
        LogLevel.ERROR,
  ...
  describe("json error", () => {
    it("circular reference", () => {
      ...
      expect(json.circular).to.eql({
        name: "child",
        parent: CIRCULAR_STRUCTURE_ERROR,
      });
    });
  });
});

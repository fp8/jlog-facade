import { LoggerFactory, LogLevel } from "@fp8proj";
...
describe("dest-filter", () => {
  ...
  it("from config info log", () => {
    ...
    expect(logCollector).to.eql(["|I loggerA info IBCDEFLPFa"]);
  });
  ...
  it("from config error log - with dest logger", () => {
    ...
    expect(logCollector).to.eql(["|E loggerA error jTo2L0k0Q9"]);
  });
});
